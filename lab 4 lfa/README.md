...
This laboratory provided valuable insight into how abstract regex patterns become executable code. Building a recursive-descent parser transformed the requirement to "dynamically interpret" regex from a vague concept into concrete understanding: it means handling regex as data that must be parsed and executed at runtime, not hardcoded logic. The experience highlighted the power of abstraction—separating parsing from generation via an AST allowed clean handling of complex expressions. A key lesson was recognizing the gap between mathematical ideals (infinite languages) and practical implementation (bounded repetitions). Adding tracing revealed that observability during algorithm execution is essential for verification and debugging. Overall, this lab bridged theory and practice effectively, demonstrating that foundational concepts like parsing and code generation—though academic in appearance—directly underpin tools we use daily.
